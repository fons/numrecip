...
trait ClampedSpline extends SplineStrategyT with LinearSystemsSolverT {
    ...
    def strategy(indep: Vector[Double], data: Vector[Double]): Option[Vector[Double]] = {
      ...
      val c0   = nc(0)(0)
      val nc1  = nc.updated(0, nc(0).updated(0, c0 - 3.0 * (rat_yx(0) - derivs.first )))
      println("===================================")
      println(nc)
      println(deriv0, derivN, rat_yx(0))
      println(nc1)
      println("===================================")
      val last = nc1.length - 1
      val cN   = nc1(last)(0)
      val nc2  = nc1.updated(last, nc1(last).updated(0, cN - 3.0 * (derivs.last - rat_yx(rat_yx.length -1) )))
